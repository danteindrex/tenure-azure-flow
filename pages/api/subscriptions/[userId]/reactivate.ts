...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    // Security: Users can only reactivate their own subscription
    if (session.user.id !== userId) {
      return res.status(403).json({ error: 'Forbidden: Cannot reactivate another user\'s subscription' });
    }

    // Forward to subscription service
    const subscriptionServiceUrl = process.env.SUBSCRIPTION_SERVICE_URL || 'http://localhost:3001';

    console.log(`Forwarding reactivate request to: ${subscriptionServiceUrl}/api/subscriptions/${userId}/reactivate`);

    const serviceResponse = await fetch(`${subscriptionServiceUrl}/api/subscriptions/${userId}/reactivate`, {
      method: 'POST',
      headers: {
