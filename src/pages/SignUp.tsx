...
import { Crown, Check, ChevronRight, ChevronLeft, Loader2, Mail, Phone } from "lucide-react";
...
const SignUp = () => {
  ...
  const [step, setStep] = useState(1); // 1: Email+Password, 2: Email OTP, 3: Phone, 4: Phone OTP, 5: Personal Info, 6: Payment
    ...
    try {
      ...
      if (session?.user) {
        ...
        if (userData?.signup_step) {
          ...
          if (currentStep === 1 && !userData.email_verified) {
            setStep(2); // Go to email verification
            toast.info("Continue your signup by verifying your email");
          } else if (currentStep === 2 && userData.email_verified && !userData.phone) {
            setStep(3); // Go to phone collection
            toast.info("Continue your signup by adding your phone number");
          } else if (currentStep === 3 && userData.phone && !userData.phone_verified) {
            setStep(4); // Go to phone verification
            toast.info("Continue your signup by verifying your phone number");
          } else if (currentStep === 4 && userData.phone_verified && !userData.profile_completed) {
            setStep(5); // Go to profile completion
  ...
  const handleOtpInputChange = (field: 'emailOtpCode' | 'phoneOtpCode', value: string): void => {
  ...
  const handleStep1Submit = async (): Promise<void> => {
    ...
    try {
      ...
      const { data: authData, error: authError } = await supabase.auth.signUp({
        ...
        options: {
          data: {
            ...
            signup_method: 'email_first',
            terms_agreed: formData.agreeToTerms,
          },
          emailRedirectTo: `${window.location.origin}/signup?step=2`
        }
      });
      ...
      await logSignup(email, true, authData.user?.id);
    ...
    try {
      ...
      const { error } = await supabase.auth.signInWithOtp({
        ...
        options: {
          ...
          data: {
            verification_type: 'email_signup'
          }
        }
      });
  ...
  const handleEmailOtpVerification = async (): Promise<void> => {
    ...
    try {
      ...
      setStep(3);
      toast.success("Email verified successfully! Please add your phone number.");
  ...
  const handleStep5Submit = async (): Promise<void> => {
    ...
    try {
      ...
      // Call profiles/upsert API to save to normalized tables
      ...
      if (!profileResponse.ok) {
        ...
        await logError(`Profile upsert failed: ${errorData.error}`, userId || undefined);
        toast.error(`Failed to save profile: ${errorData.error}`);
        return;
      }

      // Update user metadata for signup tracking
      await supabase.auth.updateUser({
        data: {
          signup_step: 5,
          profile_completed: true,
          signup_method: 'email_first_complete'
        }
      });

      // Move to payment step
      setStep(6);
      toast.success("Profile completed! Please proceed to payment.");
