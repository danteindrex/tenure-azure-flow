...
export const QueueRow = ({ rank, name, tenureMonths, status, isCurrentUser, index }: QueueRowProps) => {
  const animatedRank = useCounterAnimation(rank, 800, index * 100);
  const animatedMonths = useCounterAnimation(tenureMonths, 1000, index * 100 + 50);
  ...
  return (
    ...
    >
      <td className="py-4 font-mono">{animatedRank}</td>
      <td className="py-4 font-medium">{name}</td>
      <td className="py-4">{animatedMonths} months</td>
      <td className="py-4">
