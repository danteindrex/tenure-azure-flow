-- Create missing tables for admin dashboard collections
...
CREATE TABLE IF NOT EXISTS admin_alerts (
  ...
  trigger_info JSONB,
  assigned_to UUID REFERENCES admin(id),
  acknowledged_by UUID REFERENCES admin(id),
  acknowledged_at TIMESTAMPTZ,
  resolved_by UUID REFERENCES admin(id),
...
CREATE TABLE IF NOT EXISTS kyc_verification (
  ...
  reviewer_id UUID REFERENCES admin(id),
  reviewer_notes TEXT,
...
FROM users u
JOIN membership_queue mq ON u.id = mq.user_id
LIMIT 10;
